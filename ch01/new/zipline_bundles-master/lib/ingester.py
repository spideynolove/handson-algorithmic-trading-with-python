...
class csv_ingester(ingester_base):
    ...
    def get_csvdir(csvdir, csvdir_env, show_progress=False):
        ...
        if csvdir_env:
            error_msg = "{}. It can be set via environment variable '{}'".format(
                error_msg, csvdir_env)
        if error_msg[-1] == "'":
            error_msg = "{}. It can also be set inside '.zipline/extension.py' where the bundle is registered by setting argument 'csvdir'".format(
                error_msg)
        else:
            error_msg = "{}. It can be set inside '.zipline/extension.py' where the bundle is registered by setting argument 'csvdir'".format(
                error_msg)
        ...
        with maybe_show_progress(symbols, show_progress, label='Loading csv files: ') as it:
            for symbol_index, symbol in enumerate(it):
                ...
                if os.path.exists(file_path):
                    ...
                    df_data = pd.read_csv(file_path, index_col=self._index_column, parse_dates=True, dtype={
                                          "Volumn": np.uint64}).sort_index()
    ...
    def __call__(self,
        ...
        if self._every_min_bar:
            minute_bar_write.write(self._read_and_convert(
                symbols, show_progress), show_progress=show_progress)
        else:
            daily_bar_writer.write(self._read_and_convert(
                symbols, show_progress), show_progress=show_progress)
        if show_progress:
...
class direct_ingester(ingester_base):
    ...
    def __init__(self, exchange, every_min_bar, symbol_list_env, downloader, symbol_list=None, filter_cb=None):
        ...
        """
        super().__init__(exchange, every_min_bar)
        self._symbols = direct_ingester.create_symbol_list(
            symbol_list_env, symbol_list)
    ...
    def create_symbol_list(symbol_list_env, symbol_list, show_progress=False):
        ...
        if symbol_list_env:
            ...
            symbols = symbols.union(
                [sym for sym in env_list.split(',') if sym.strip()])

        # add symbols from |symbol_list|
        if symbol_list:
            symbols = symbols.union(
                [sym for sym in symbol_list if sym.strip()])
    ...
    def __call__(self,
        ...
        if self._every_min_bar:
            minute_bar_write.write(self._read_and_convert(
                calendar, show_progress), show_progress=show_progress)
        else:
            daily_bar_writer.write(self._read_and_convert(
                calendar, show_progress), show_progress=show_progress)
        if show_progress:
