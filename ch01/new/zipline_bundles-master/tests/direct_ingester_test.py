...
class DirectIngesterTestCase(unittest.TestCase):
    ...
    def test_create_symbol_list(self):
        self.assertEqual(
            len(ig.direct_ingester.create_symbol_list('SYM_LIST', [])), 0)
        self.assertEqual(
            len(ig.direct_ingester.create_symbol_list('SYM_LIST', None)), 0)
    ...
    def test_read_and_convert(self):
        ...
        ingester = ig.direct_ingester(
            'EXX', False, None, downloader, symbol_list=('A', 'B'))
        ...
        def find_sym_idx(df, sym): return df[df.symbol == sym].index[0]
